...
- (void)newConnection:(NSNotification *)notification
{
    ...
    if( remoteFileHandle ) {
        FEHTTPConnection *connection = [[FEHTTPConnection alloc] initWithFileHandle:remoteFileHandle delegate:self];
        if( connection ) {
...
- (void)processNextRequestIfNecessary
{
    if( [self currentRequest] == nil && [requests count] > 0 ) {
        ...
        //[self processURL:[currentRequest objectForKey:@"url"] connection:[currentRequest objectForKey:@"connection"]];
    }
}
